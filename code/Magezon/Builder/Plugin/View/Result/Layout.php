...
namespace Magezon\Builder\Plugin\View\Result;
...
class Layout
{
	public function aroundRenderResult(
		...
		$regex  = '@(?:<style id="magezon-builder")(.*)</style>@msU';
        preg_match_all($regex, $html, $_matches);
        $stylesHtml = implode('', $_matches[0]);
        if ($stylesHtml) {
        	$stylesHtml = $this->minifyCss($stylesHtml);
        	$html = preg_replace($regex, '', $html);
        	$html = str_replace('</head>', $stylesHtml . '</head>', $html);
    	}
