...
/***/ function(module, exports, __webpack_require__) {
	...
	function getWatchersFromScope(scope, arrs) {
	  var result = scope && scope.$$watchers ? scope.$$watchers : [];
	  // angular.forEach(result, function(item, index) {
	  // 	if (angular.isString(item.exp)) {
		 //  	var _tmp = _.findWhere(arrs, {'id': item.exp});
		 //  	if (_tmp) {
		 //  		_tmp.count++;
		 //  	} else {
		 //  		_tmp = {'id': item.exp, 'count': 1};
		 //  		arrs.push(_tmp);
		 //  	}
		 //  }
	  // });
