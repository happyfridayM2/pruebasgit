...
class OptIn extends Action implements CsrfAwareActionInterface
{
    ...
    public function execute()
    {
        ...
        $this->subscribeAccounts($email, $checksum, $this->getSecretKeysSingleAccount());
    }
    ...
    private function subscribeAccounts($email, $request_checksum, $secretKeys)
    {
        foreach ($secretKeys as $secretKey) {
            ...
            if ($local_checksum == $request_checksum) {
                $this->_newsletterHelper->subscribeEmail($email);
            }
        }
    }
}
